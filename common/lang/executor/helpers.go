...
func NewEmptyStruct(def *StructDefinition) *ObjStruct {
	return &ObjStruct{
...
func (e *Environment) LoadVarsInStruct(definition *StructDefinition, s map[string]interface{}) *ObjStruct {
	fields := make(map[string]Object)
	for k, v := range s {
...
func getLangType(t interface{}) ObjectType {
	switch t.(type) {
	case float64:
		return TypeFloat
	case int:
